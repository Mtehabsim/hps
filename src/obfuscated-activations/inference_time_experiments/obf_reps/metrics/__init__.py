...
class MetricConfig:
    ...
    # ---- HPS-specific (used only by HPSMetric) ----
    ...
    obf_signal: str = "sigmoid" # attack-loss shaping for HPS/HHD: sigmoid|softplus|logit (all monotonic -> same detection)
    # ---- HierarchicalHyperbolicMetric-specific ----
    hhd_curvature: float = 1.0     # curvature c (c->0 ~ Euclidean); matches curvature_sweep parameterization
    hhd_embed_dim: int = 16        # encoder output / tangent dim
    hhd_leaves: int = 30           # # content leaves (JOINT harm+benign clustering); harm is a per-leaf attribute
    hhd_n_layers: int = 6          # auto-select the best N layers (by harm-separability) from config.layers candidates
    hhd_harm_clusters: int = 8     # (deprecated; kept for back-compat)
    hhd_benign_clusters: int = 8   # (deprecated; kept for back-compat)
    hhd_epochs: int = 300          # prototype-net training epochs
...
class HPSMetric(ObfMetric):
    """Hyperbolic Projection Sentinel.

    Projects activations from `config.layers` onto a Lorentz hyperboloid, computes
    12 cross-layer trajectory features (radial / curvature / displacement), and scores
    with logistic regression. Unlike TrainableMetric (independent per-layer probes),
    HPS is intrinsically multi-layer, so it subclasses ObfMetric directly (like the
    Mahalanobis / VAE metrics) and collapses the layer dimension.

    `predict()` is fully differentiable and is what ProbePredictLoss uses as the
    obfuscation loss during the attack.
    """

    def _selected(self, rep_layers):
    ...
    def _gather_train(self, reps_dataset) -> Tuple[Tensor, Tensor]:
        ...
        X, y = [], []
        for rep, label in reps_dataset:
            r = rep.squeeze(0)              # [layers, seq, hidden]
            sel = r[layers]                 # [n_sel, seq, hidden]
            sel = sel.mean(dim=1)           # [n_sel, hidden]
            X.append(sel.float())
            y.append(float(label.reshape(-1)[0].item()))
        return torch.stack(X), torch.tensor(y)

    def _features_from_X(self, X: Tensor) -> Tensor:
        """X: [b, n_sel, hidden] -> [b, 12] (differentiable)."""
        z = [self.proj(X[:, l, :]) for l in range(X.shape[1])]  # each [b, 1+d_proj]
        z = torch.stack(z, dim=0)                               # [n_sel, b, 1+d_proj]
        return hps_trajectory_features(z, self.kappa)

    def _logits(self, reps: Float[Tensor, "b layers seq hidden"]) -> Float[Tensor, "b"]:
        if reps.ndim == 3:
            reps = reps.unsqueeze(2)
        layers = list(self.config.layers)
    ...
    def fit(self, reps_dataset: RepsDatasetT, **kwargs):
        device = self.device
        kappa = float(getattr(self.config, "kappa", 0.1))
        proj_dim = int(getattr(self.config, "proj_dim", 64))
        epochs = int(getattr(self.config, "hps_epochs", 50))

        X, y = self._gather_train(reps_dataset)
        X = X.to(device)
        y = y.to(device)
        ...
        for _ in range(epochs):
            ...
            kval = float(proj.k.item())
            for li in range(n_sel):
                h = proj(X[:, li, :])
                n = h.shape[0]
                idx = torch.randperm(n, device=device)[:512]
                hb = h[idx]
                yb = y[idx]
                d = lorentz_distance(hb.unsqueeze(1), hb.unsqueeze(0), kval)
                ...
                loss = (
                    (d**2 * same * triu).sum() / ns
                    + (torch.clamp(margin - d, min=0) ** 2 * (1 - same) * triu).sum() / nd
                ) / 2
                loss_total = loss_total + loss
            loss_total = loss_total / n_sel
            opt.zero_grad()
            loss_total.backward()
            opt.step()
        proj.eval()
        ...
        self.proj = proj
        self.kappa = float(proj.k.item())

        # ---- Fit StandardScaler + LR on trajectory features ----
        ...
        self.lr_weight = torch.tensor(clf.coef_[0], dtype=torch.float32, device=device)
        self.lr_bias = torch.tensor(float(clf.intercept_[0]), dtype=torch.float32, device=device)

        try:
            auroc = float(
                __import__("sklearn.metrics", fromlist=["roc_auc_score"]).roc_auc_score(
                    y.cpu().numpy(), clf.decision_function(feats_std)
                )
            )
            print(f"    HPS train AUROC = {auroc:.4f}")
        except Exception:
            pass

    def _obf_score(self, reps):
        # Score from the HPS logit; the attack MINIMIZES this as its obfuscation loss.
        #   sigmoid : bounded (0,1), scale-matched to C4 -- BUT saturates at confident-
        #             harmful (grad p(1-p)->0) => GRADIENT MASKING, giving HPS spurious
        #             'resistance' at high obf weight (low lambda).
        #   softplus: non-saturating benign cross-entropy (grad ->1 as logit->+inf),
        #             monotonic => detection AUROC/recall UNCHANGED, but attack gradient
        #             no longer vanishes. This is the fair, un-masked obfuscation signal.
        #   logit   : raw (non-saturating, large scale).
        # All choices are monotonic in the logit => identical detection metrics.
        logit = self._logits(reps)
        sig = str(getattr(self.config, "obf_signal", "sigmoid"))
        if sig == "logit":
            return logit
        if sig == "softplus":
    ...
    def evaluate(
        self, reps: Float[Tensor, "b layers seq_len hidden_dim"], **kwargs
    ) -> Float[Tensor, "b layers seq_len"]:
        if reps.ndim == 3:
            reps = reps.unsqueeze(2)
        b, layers, seq_len, _ = reps.shape
        scores = self._obf_score(reps)  # [b]
    ...
    def load_pretrained(self, path: Optional[str] = None, probe_state: Optional[Any] = None):
        ...
        state = torch.load(path) if path is not None else probe_state
        proj = LorentzProjection(state["d_in"], state["proj_dim"], k_init=state["kappa"]).to(
            self.device
        )
        ...
        self.lr_bias = state["lr_bias"].to(self.device)
...
class HierarchicalHyperbolicMetric(ObfMetric):
    """Hierarchical-hyperbolic detector wrapped as an obf_reps metric so it can be
    ADAPTIVELY ATTACKED (soft-prompt / GCG), not just scored on clean data.

    DESIGN (principled hierarchy; harm/benign is a LEAF ATTRIBUTE, not the top split):
      fit() concatenates ALL configured layers, then clusters ALL training reps JOINTLY
      by *content* into K leaves (KMeans) -- harm and benign are NOT clustered separately,
      so the harm/benign decision is not baked into the tree. Each leaf l carries a
      harm-fraction h_l (from the train labels). A curvature-c Lorentz prototype net
      (linear encoder -> exp-map -> per-leaf distance) learns to place a point near its
      *content* leaf; the harm score is then read off WHICH leaf the point falls in:
          w   = softmax(-dist to the K leaf prototypes)         # soft leaf assignment
          harm= sum_l w_l * h_l                                 # expected harm over leaves
          margin = logit(harm) = log(harm) - log(1-harm)        # >0 => harmful
      This gives the geometry "room" to organize what the text is about; harm/benign
      emerges from leaf membership (if content leaves are pure, h_l ~ 0/1 -- the data's
      own structure, per Q5c). Curvature c uses the curvature_sweep parameterization
      (c->0 ~ Euclidean).

    MASKING-SAFE BY CONSTRUCTION (so an 'it resists' result can be trusted, cf. the HPS
    sigmoid trap at lambda=0.1):
      * tangent norm soft-clamped via 30*tanh(n/30) (no zero-gradient region; overflow-safe).
      * attack obf loss (predict) uses obf_signal=softplus (non-saturating) by default;
        obf_signal=sigmoid only to DEMONSTRATE masking.
      * evaluate() returns the RAW monotonic margin -> detection AUROC is signal-independent.
    Both predict() and evaluate() are differentiable w.r.t. the input reps.
    """

    # ---- curvature-c Lorentz ops (curvature-1 hyperboloid, distance scaled by 1/sqrt(c)) ----
    def _expmap(self, v: Tensor) -> Tensor:
        c = self._c
        u = (c ** 0.5) * v
        n = u.norm(dim=-1, keepdim=True).clamp_min(1e-8)
        un = 30.0 * torch.tanh(n / 30.0)          # SOFT clamp (cap 30, cosh(30)~5e12 fp32-safe) -> gradient always flows; saturation far outside any realistic encoder output (masking-safe)
    ...
    def _margin_from_Xs(self, Xs: Tensor) -> Tensor:
        """Xs: standardized [b, D] (D = n_layers * hidden). Returns harm log-odds [b]."""
        xc = self._expmap(Xs @ self.W_enc + self.b_enc)   # [b, 1+d_emb]
        pc = self._expmap(self.proto)                     # [K, 1+d_emb]
        w = torch.softmax(-self._dist(xc, pc), dim=1)     # [b, K] soft leaf assignment
        harm = (w @ self.leaf_harm).clamp(1e-4, 1 - 1e-4) # expected harm over leaves
        return torch.log(harm) - torch.log(1 - harm)      # logit(harm), >0 => harmful

    def _prep(self, reps: Tensor) -> Tensor:
        """reps [b, layers, seq, hidden] -> standardized [b, k*hidden] (the best-k selected layers concatenated)."""
        if reps.ndim == 3:
            reps = reps.unsqueeze(2)
        sel = reps[:, self.sel_layers, :, :].to(torch.float32)  # [b, k, seq, hidden]
        X = sel.mean(dim=2)                                     # [b, k, hidden] (mean over seq)
        X = X.reshape(X.shape[0], -1)                           # [b, k*hidden] (CONCAT selected layers)
        X = torch.nan_to_num(X, nan=0.0, posinf=0.0, neginf=0.0)
        return (X - self.act_mean) / self.act_std

    def _margin(self, reps: Tensor) -> Tensor:
        return self._margin_from_Xs(self._prep(reps))

    def _obf_score(self, reps: Tensor) -> Tensor:
        m = self._margin(reps)
        sig = str(getattr(self.config, "obf_signal", "softplus"))
        if sig == "logit":
            return m
        if sig == "sigmoid":           # saturating -> only to DEMONSTRATE masking
            return torch.sigmoid(m)
        return torch.nn.functional.softplus(m)   # default: non-saturating (masking-safe)

    def _gather(self, reps_dataset) -> Tuple[Tensor, Tensor]:
        layers = list(self.config.layers)
        X, y = [], []
        for rep, label in reps_dataset:
            r = rep.squeeze(0)                            # [layers, seq, hidden]
            sel = r[layers].mean(dim=1)                   # [n_cand, hidden] (mean over seq)
            X.append(sel.float())
            y.append(float(label.reshape(-1)[0].item()))
        return torch.stack(X), torch.tensor(y)            # [N, n_cand, hidden], [N]

    def fit(self, reps_dataset: RepsDatasetT, **kwargs):
        from sklearn.cluster import KMeans
        from sklearn.metrics import roc_auc_score
        device = self.device
        self._c = float(getattr(self.config, "hhd_curvature", 1.0))
        d_emb = int(getattr(self.config, "hhd_embed_dim", 16))
        K = int(getattr(self.config, "hhd_leaves", 30))
        n_lay = int(getattr(self.config, "hhd_n_layers", 6))
        epochs = int(getattr(self.config, "hhd_epochs", 300))

        Xc, y = self._gather(reps_dataset)                # [N, n_cand, hidden]
        Xc = torch.nan_to_num(Xc.to(device), nan=0.0, posinf=0.0, neginf=0.0)
        yn = y.cpu().numpy(); cand = list(self.config.layers)

        # ---- rank candidate layers by linear harm-separability (mean-diff AUROC); keep best n_lay ----
        aucs = []
        for i in range(Xc.shape[1]):
            Xi = Xc[:, i, :]
            Xi = (Xi - Xi.mean(0)) / (Xi.std(0) + 1e-6)
            d = Xi[y == 1].mean(0) - Xi[y == 0].mean(0)
            try:
                a = roc_auc_score(yn, (Xi @ d).cpu().numpy())
            except Exception:
                a = 0.5
            aucs.append(max(a, 1 - a))
        k = min(n_lay, len(cand))
        sel_pos = sorted(int(p) for p in np.argsort(aucs)[::-1][:k])
        self.sel_layers = [cand[p] for p in sel_pos]
        print(f"    HHD selected best {k} layers {self.sel_layers} "
              f"(per-layer sep-AUROC {[round(aucs[p], 3) for p in sel_pos]})", flush=True)

        Xsel = Xc[:, sel_pos, :].reshape(Xc.shape[0], -1)  # [N, k*hidden]
        self.act_mean = Xsel.mean(0)
        self.act_std = Xsel.std(0, unbiased=False).clamp(min=1e-6)
        Xs = (Xsel - self.act_mean) / self.act_std

        # JOINT content clustering (harm + benign together); harm is a per-leaf attribute.
        Xn = Xs.detach().cpu().numpy()
        K = min(K, len(Xn))
        leaf = KMeans(K, n_init=4, random_state=0).fit(Xn).labels_
        leaf_harm = np.array([yn[leaf == l].mean() if (leaf == l).any() else 0.5 for l in range(K)])
        self.leaf_harm = torch.tensor(leaf_harm, dtype=torch.float32, device=device)
        purity = float(np.mean(np.maximum(leaf_harm, 1 - leaf_harm)))

        D = Xs.shape[1]
        torch.manual_seed(0)
        W = torch.nn.Parameter(torch.randn(D, d_emb, device=device) / D ** 0.5)
        b = torch.nn.Parameter(torch.zeros(d_emb, device=device))
        proto = torch.nn.Parameter(torch.randn(K, d_emb, device=device) * 0.1)
        self.W_enc, self.b_enc, self.proto = W, b, proto
        opt = torch.optim.Adam([W, b, proto], lr=1e-2, weight_decay=1e-4)
        yt = torch.tensor(leaf, device=device).long(); ce = torch.nn.CrossEntropyLoss()
        for _ in range(epochs):  # learn to classify CONTENT leaf membership (not harm directly)
            logit = -self._dist(self._expmap(Xs @ W + b), self._expmap(proto))
            opt.zero_grad(); ce(logit, yt).backward()
            torch.nn.utils.clip_grad_norm_([W, b, proto], 5.0); opt.step()
        self.W_enc = W.detach(); self.b_enc = b.detach(); self.proto = proto.detach()
        try:
            with torch.no_grad():
                m = self._margin_from_Xs(Xs).cpu().numpy()
            print(f"    HHD train AUROC = {roc_auc_score(yn, m):.4f} "
                  f"(c={self._c}, {K} content leaves, leaf-purity={purity:.3f}, "
                  f"best {k} layers, D={D}, d_emb={d_emb})", flush=True)
        except Exception as e:
            print(f"    HHD train AUROC n/a ({e})", flush=True)

    def evaluate(self, reps: Float[Tensor, "b layers seq_len hidden_dim"], **kwargs
                 ) -> Float[Tensor, "b layers seq_len"]:
        if reps.ndim == 3:
            reps = reps.unsqueeze(2)
        b, layers, seq_len, _ = reps.shape
        m = self._margin(reps)                           # raw monotonic margin (signal-independent)
        return m.view(b, 1, 1).expand(b, layers, seq_len).to(torch.float16)

    def predict(self, reps: Float[Tensor, "b layers seq_len hidden_dim"],
                attention_mask: Optional[Bool[Tensor, "b seq_len"]] = None,
                layer_reduction: str = "mean", **kwargs) -> Float[Tensor, "b"]:
        return self._obf_score(reps)

    def load_pretrained(self, path: Optional[str] = None, probe_state: Optional[Any] = None):
        assert path is not None or probe_state is not None
        s = torch.load(path) if path is not None else probe_state
        self._c = float(s["c"])
        self.sel_layers = list(s["sel_layers"])
        self.W_enc = s["W_enc"].to(self.device); self.b_enc = s["b_enc"].to(self.device)
        self.proto = s["proto"].to(self.device); self.leaf_harm = s["leaf_harm"].to(self.device)
        self.act_mean = s["act_mean"].to(self.device); self.act_std = s["act_std"].to(self.device)

    def save_probe(self, path: Optional[str] = None) -> None | Any:
        state = {"c": self._c, "sel_layers": self.sel_layers, "W_enc": self.W_enc,
                 "b_enc": self.b_enc, "proto": self.proto, "leaf_harm": self.leaf_harm,
                 "act_mean": self.act_mean, "act_std": self.act_std}
        if path is not None:
            torch.save(state, path); return None
        return state
