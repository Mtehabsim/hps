        ...
        for i in range(0, input_embeds.shape[0], search_batch_size):
            with torch.no_grad():
                ...
                if self.use_prefix_cache:
                    ...
                    prefix_cache_batch = []
                    for j in range(len(self.prefix_cache)):
                        prefix_cache_batch.append([])
                        for k in range(len(self.prefix_cache[j])):
                            prefix_cache_batch[j].append(
                                self.prefix_cache[j][k].expand(current_batch_size, -1, -1, -1)
                            )

                # Candidate evaluation needs only loss VALUES (no backprop): keep the
                # forward + loss under no_grad. The original code left these OUTSIDE the
                # no_grad block, building a full autograd graph over all candidates ->
                # OOM ("No executable batch size found, reached zero").
