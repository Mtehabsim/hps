...
def evaluate_model_and_metric_harmful(
    ...
    if evaluate_behavior:
        ...
        for batch_num, batch in enumerate(tqdm.tqdm(positive_dataloader)):
            ...
            # --- Optional: dump per-example activations for visualization ---
            # Enable with env OBF_DUMP_ACTS=N. Saves last-token activations at the
            # metric's layers for the first N harmful examples: attacked (with soft
            # prompt) vs clean (no attack). Lets us SEE the attack move activations.
            try:
                import os as _os2
                _Ndump = int(_os2.environ.get("OBF_DUMP_ACTS", "0"))
            except Exception:
                _Ndump = 0
            if _Ndump > 0 and who_just_went == "attacker":
                try:
                    import numpy as _np2
                    _layers = list(metric.config.layers)
                    _att = tuned_generate.input_reps[:, _layers, -1, :].detach().float().cpu().numpy()
                    _cln = standard_teacher_forced.input_reps[:, _layers, -1, :].detach().float().cpu().numpy()
                    global _ACT_DUMP
                    try:
                        _ACT_DUMP
                    except NameError:
                        _ACT_DUMP = {"prompt": [], "score": [], "attacked": [], "clean": []}
                    for _i in range(len(input_text)):
                        if len(_ACT_DUMP["prompt"]) >= _Ndump:
                            break
                        _ACT_DUMP["prompt"].append(input_text[_i])
                        _ACT_DUMP["score"].append(float(behavior_score_batch[_i]))
                        _ACT_DUMP["attacked"].append(_att[_i])
                        _ACT_DUMP["clean"].append(_cln[_i])
                    _os2.makedirs("runs", exist_ok=True)
                    _ap = f"runs/acts_{metric.__class__.__name__}_{_os2.getpid()}.npz"
                    _np2.savez(
                        _ap,
                        layers=_np2.array(_layers),
                        prompt=_np2.array(_ACT_DUMP["prompt"], dtype=object),
                        score=_np2.array(_ACT_DUMP["score"]),
                        attacked=_np2.array(_ACT_DUMP["attacked"]),
                        clean=_np2.array(_ACT_DUMP["clean"]),
                    )
                except Exception as _e:
                    print(f"[dump-warn] acts: {_e}", flush=True)
            end = time.time()
